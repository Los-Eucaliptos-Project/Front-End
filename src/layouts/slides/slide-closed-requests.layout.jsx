...
function SlideDashboard() {
  ...
  const updatedMenuItems = MenuItems.map(item => {
    if (item.href === '/closed-requests') {
      return { ...item, onClick: () => handleRefresh('/closed-requests'), showGreenBar: true, greenBarMarginTop: '180px' };
    }
    return item;
  });
  ...
  return (
    <SlideContainer $isMobile={isMobile}>
      <div className={styles.slideContent}>
        ...
        <SlideMenu isMobile={isMobile} isMenuOpen={isMenuOpen} handleRefresh={handleRefresh} menuItems={updatedMenuItems} selectedPath="/closed-requests" />
        <SlideBarGreen marginTop="450px" />
      </div>
    </SlideContainer>
  );
}
